<!-- source: /home/reza/projects/game/docs/UnityDocumentation/Documentation/en/ScriptReference/Unity.Jobs.IJobParallelFor.html
...
This job type mainly exists for backwards compatibility. New code should use IJobFor instead, which exposes the same functionality through a more consistent API. This job type has the following options to schedule work:

- IJobForExtensions.RunByRef runs the job on the main thread and blocks until it is complete.
- IJobForExtensions.ScheduleByRef schedules the job to run asynchronously, distributing the work across multiple worker threads. This scheduling option can give the best performance, but race conditions can occur if the job writes to shared or global data without proper synchronization.

`Execute(int index)` is executed once for each index from 0 to the provided length.

`RunByRef` guarantees that the the job's `Execute(int index)` method is invoked sequentially. `ScheduleByRef` doesn't invoke the job's `Execute` method sequentially because it's called from multiple worker threads in parallel to each other.
