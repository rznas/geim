<!-- source: /home/reza/projects/game/docs/UnityDocumentation/Documentation/en/ScriptReference/Texture2D.GetPixels32.html
...
using UnityEngine;public class Texture2DExample : MonoBehaviour
{
    ...
    public Texture2D destination;    void Start()
    {
        ...
        Color32[] pixels = source.GetPixels32(sourceMipLevel);        // If required, manipulate the pixels before applying them to the destination Texture2D.
        ...
        destination.SetPixels32(pixels, destinationMipLevel);        // Apply changes to the destination Texture2D, which uploads its data to the GPU.
        destination.Apply();
    }
}
```
