<!-- source: /home/reza/projects/game/docs/UnityDocumentation/Documentation/en/ScriptReference/Rendering.CommandBuffer.DispatchRays.html
...
void MainRayGenShader()
{
    ...
    payload.color = float4(0, 0, 0, 0);    uint missShaderIndex = 0;
...
In this ray generation shader example, you calculate a ray direction based on the 2D thread index returned by the `DispatchRaysIndex()` function. The output is red if there is a ray/triangle intersection. When there is no intersection, the GPU executes MainMissShader and the output is blue. This example uses the `unity_CameraToWorld` built-in shader variable. You must specify a Camera as an argument for the `DispatchRays` function to set this value correctly.
