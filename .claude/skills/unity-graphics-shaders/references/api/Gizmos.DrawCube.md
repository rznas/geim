<!-- source: /home/reza/projects/game/docs/UnityDocumentation/Documentation/en/ScriptReference/Gizmos.DrawCube.html
...
Draws a solid box at `center` with `size` in the Scene view for visualization and debugging purposes.

`DrawCube` is a method used for visual debugging in the Unity Editor. Use `DrawCube` to render a solid-color cube in the Scene view. `DrawCube` can be useful for visualizing bounding boxes, trigger areas, or spatial relationships between objects.

The cube is drawn using the current Gizmos color, which can be set using Gizmos.color before calling `DrawCube`. The cube is drawn using the current Gizmos matrix, which can be set using Gizmos.matrix before calling `DrawCube`.
...
using UnityEngine;public class CubeGizmoExample : MonoBehaviour
{
    ...
    public float alpha = 0.5f;    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawCube(transform.position, size);        // Draw a wire cube outline.
