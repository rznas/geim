<!-- source: /home/reza/projects/game/docs/UnityDocumentation/Documentation/en/ScriptReference/TransformHandle-eulerAngles.html
...
TransformHandle.eulerAngles represents rotation in world space. When viewing the rotation of a GameObject in the Inspector, you may see different angle values from those stored in this property. This is because the Inspector displays local rotation, for more information see TransformHandle.localEulerAngles. 
...
public class ExampleScript : MonoBehaviour
{
    ...
    float z;    void Update()
    {
        ...
        currentEulerAngles += new Vector3(x, y, z) * Time.deltaTime * rotationSpeed;        //apply the change to the gameObject
        TransformHandle thisTransformHandle = transformHandle;
        thisTransformHandle.eulerAngles = currentEulerAngles;
    }    void OnGUI()
    {
        GUIStyle style = new GUIStyle();
        style.fontSize = 24;
        GUI.Label(new Rect(10, 0, 0, 0), "Rotating on X:" + x + " Y:" + y + " Z:" + z, style);        GUI.Label(new Rect(10, 25, 0, 0), "transformHandle.eulerAngle: " + transformHandle.eulerAngles, style);
    }
}
```

Do not set one of the eulerAngles axis separately (eg. eulerAngles.x = 10; ) since this will lead to drift and undesired rotations. When setting them to a new value set them all at once as shown above. Unity converts the angles to and from the rotation stored in TransformHandle.rotation.

Angles are expressed modulo 360 degrees. For instance, specifying 1, 361, or -17999 results in the same angle, as demonstrated in the previous example, where currentEulerAngles increase over time beyond 360 for x, y, and z.
